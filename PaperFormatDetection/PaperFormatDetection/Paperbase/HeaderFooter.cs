        ...
        */
        protected void detectFirstSection(WordprocessingDocument docx)
        {
            ...
            foreach (HeaderReference headr in headrs)
            {
                ...
                {
                    if (header.InnerText.Replace(" ","").Length > 0)
                    {
            ...
            for (int i = 2; i <= list.Count<SectionProperties>(); i++)
            {
                ...
                foreach (HeaderReference headr in headrs)
                {
                    ...
                    if (header.InnerText != null)
                    {
                        ...
                        {
                            PaperFormatDetection.Tools.Util.printError("第" + i + "节"+headertype + "页眉字体错误，应为:" + CNheaderFonts);
                        }
                        if (Util.correctsize(p, docx, headerFontsize) == false)
                        {
                            PaperFormatDetection.Tools.Util.printError("第" + i + "节" + headertype+"页眉字号错误，应为：" + headerFontsize);
                        }
                        if (JustificationCenter(p, Mpart) == false)
                        {
                            PaperFormatDetection.Tools.Util.printError("第" + i + "节" + headertype + "页眉未居中");
                        }
                    }
                }
                //页脚
                ...
                foreach (FooterReference footr in footrs)
                {
                    ...
                    if (footer.InnerText.Trim() != "")
                    {
                        ...
                        {
                            Util.printError("第" + i + "节"+type+"页脚字体错误，应为：" + CNFooterFonts );
                        }
                        if (Util.correctsize(p, docx, footerFontsize) == false)
                        {
                            Util.printError("第" + i + "节" + type + "页脚字号错误，应为：" + footerFontsize );
                        }
                        if (JustificationCenter(p, Mpart) == false)
                        {
                            Util.printError("第" + i + "节" + type + "页脚未居中");
                        }
                    }
                }
            }
            ...
            for (int i = 2; i <= list.Count<SectionProperties>(); i++)
            {
                ...
                if (headerdefault != null)
                {
                    ...
                    if (header.InnerText != null)
                    {
                        ...
                        {
                            Util.printError("第" + i + "节" + "奇数页" + "页眉内容错误，应为：" +  oddHeaderText );
                        }
                    }
                }
                //偶数页
                if (headereven != null)
                {
                    ...
                    if (header.InnerText != null)
                    {
                        if(header.InnerText!="")
                        {
                            ...
                            {
                               Util.printError("第" + i + "节" + "偶数页"+"页眉内容错误，应为：" + evenHeaderText);
                            }
                        }
                    }
                }                            
