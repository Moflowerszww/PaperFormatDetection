    ...
    class Punctuation
    {
        ...
        public void CNdetect(string str)
        {
            ...
            if (mc.Count > 0)
            {
                ...
                {
                    if(!match.ContainsKey(m.Index))
                        match.Add(m.Index, m.ToString().Length);
                }
            }
            //去掉[1, 2-3]这样的
            mc = Regex.Matches(str, @"[\(|\[][0-9]+([,][ ]*[0-9]+)+([-][0-9]+)*[\)|\]]");
            if (mc.Count > 0)
            {
                foreach (Match m in mc)
                {
            ...
            //H. h
            mc = Regex.Matches(str, @"[a-zA-Z0-9]+[.][ ]*[a-zA-Z0-9]+");
            if (mc.Count > 0)
            {
                foreach (Match m in mc)
                {
            ...
            mc = Regex.Matches(str, @"[0-9a-zA-Z][(][)]");
            if (mc.Count > 0)
            {
                foreach (Match m in mc)
                {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                if (Array.IndexOf(CNtemplate, array[i]) > -1)
                {
                    ...
                    if (leftIsLetter(str, i) && rightIsLetter(str, i))
                        continue;
                    if ((array[i] == '.') && (rightIsLetter(str, i) || leftIsLetter(str, i)))
                        continue;
        ...
        public bool leftIsLetter(string str, int index)
        {
            while (index - 1 >= 0)
            {
                ...
                {
                    if (Regex.IsMatch(str.Substring(index - 1, 1), @"[^\u4e00-\u9fa5]"))
                        return true;
                    else
                        return false;
                }
        ...
        public string noteHandle(string str)
        {
            ...
            if (mc.Count > 0)
            {
                ...
                {
                    str = str.Replace(m.ToString(), "");
                    string temp = m.ToString().Replace("\"", "").Replace("\"", "").Replace("'", "");
