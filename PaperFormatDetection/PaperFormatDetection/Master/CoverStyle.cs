...
namespace PaperFormatDetection.Master
{
    class MasCoverStyle
    {
        ...
        public string MasterType;//论文类型（专硕还是学硕）

        public MasCoverStyle()
        {
        }
        //论文封面检测
        public void detectCoverStyle(List<Paragraph> list, WordprocessingDocument doc)
        {
            ...
            else
            {
                ...
                {
                    i++;
                    if ((Util.getFullText(list[i]).Trim().Length != 0 && (Util.getFullText(list[i]).Replace(" ", "").IndexOf("硕士") != -1 || Util.getFullText(list[i]).Replace(" ", "").IndexOf("学位") != -1 || Util.getFullText(list[i]).Replace(" ", "").IndexOf("论文") != -1))) break;
                ...
                if (YFlag == 0)
                {
                    ...
                    if (!(Util.getFullText(list[i]).Trim().Length - 6 == int.Parse(coverstyleHeadline[1]) || Util.getFullText(list[i]).Trim().Length - 10 == int.Parse(coverstyleHeadline[1]) + 4)) Util.printError("论文类型" + "“" + CovHeadline + "”" + "每个字之间应包含一个空格");
                    if (!Util.correctJustification(list[i], doc, coverstyleHeadline[2])) Util.printError("论文类型" + "“" + CovHeadline + "”" + "未居中");
                ...
                if (YFlag == 0)
                {
                    ...
                    if (!Util.correctJustification(list[i], doc, coverstyleSubtitleCh[1])) Util.printError("论文中文标题未居中");
                ...
                if (YFlag == 0 && EnFlag == 0)
                {
                    if (!Util.correctJustification(list[i], doc, coverstyleSubtitleEn[0])) Util.printError("论文英文标题未居中");
                ...
                while (YFlag == 0)
                {
                    if ((Util.getFullText(list[i]).Trim().Length != 0 && (Util.getFullText(list[i]).Replace(" ", "").IndexOf("作者姓名") != -1 || Util.getFullText(list[i]).Replace(" ", "").IndexOf("工程领域") != -1 || Util.getFullText(list[i]).Replace(" ", "").IndexOf("学科、专业") != -1 || Util.getFullText(list[i]).Replace(" ", "").IndexOf("学号") != -1 || Util.getFullText(list[i]).Replace(" ", "").IndexOf("指导教师") != -1 || Util.getFullText(list[i]).Replace(" ", "").IndexOf("完成日期") != -1))) break;
                    i++;
                    if (i >= list.Count) { YFlag = 1; break; };
                }
                    ...
                    for (; i <= FirstStuInformation + 4; i++)
                    {
                        ...
                        if (Util.getFullText(list[i]).Trim().Length != 0)
                        {
                            if (MasterType == "专业学位硕士")
                            {
                                ...
                                if (!Util.correctSpacingBetweenLines_line(list[i], doc, coverstyleStuInformation[4])) Util.printError("学生信息中" + "“" + Z_EachStuInformation[i - FirstStuInformation] + "”" + "行行间距错误，应为" + Util.DSmap[coverstyleStuInformation[4]]);
                                if (!Util.correctSpacingBetweenLines_Be(list[i], doc, coverstyleStuInformation[5])) Util.printError("学生信息中" + "“" + Z_EachStuInformation[i - FirstStuInformation] + "”" + "行学生信息段前间距错误，应为0行");
                                if (!Util.correctSpacingBetweenLines_Af(list[i], doc, coverstyleStuInformation[6])) Util.printError("学生信息中" + "“" + Z_EachStuInformation[i - FirstStuInformation] + "”" + "行学生信息段后间距错误，应为0行");
                                if (!Util.correctfonts(list[i], doc, coverstyleStuInformation[1], "Times New Roman")) Util.printError("学生信息中" + "“" + Z_EachStuInformation[i - FirstStuInformation] + "”" + "行学生信息字体错误，应为" + coverstyleStuInformation[1]);
                                if (!Util.correctsize(list[i], doc, coverstyleStuInformation[3])) Util.printError("学生信息中" + "“" + Z_EachStuInformation[i - FirstStuInformation] + "”" + "行学生信息字号错误，应为" + coverstyleStuInformation[3]);
                            }
                            if (MasterType == "学术型硕士")
                            {
                                if (StuInformationString.IndexOf(X_EachStuInformation[i - FirstStuInformation]) == -1)
                                    Util.printError("第" + StuInformationLine + "行学生信息应为:" + X_EachStuInformation[i - FirstStuInformation]);
                                if (!Util.correctSpacingBetweenLines_line(list[i], doc, coverstyleStuInformation[4])) Util.printError("学生信息中" + "“" + X_EachStuInformation[i - FirstStuInformation] + "”" + "行行间距错误，应为" + Util.DSmap[coverstyleStuInformation[4]]);
                                if (!Util.correctSpacingBetweenLines_Be(list[i], doc, coverstyleStuInformation[5])) Util.printError("学生信息中" + "“" + X_EachStuInformation[i - FirstStuInformation] + "”" + "行学生信息段前间距错误，应为0行");
                                if (!Util.correctSpacingBetweenLines_Af(list[i], doc, coverstyleStuInformation[6])) Util.printError("学生信息中" + "“" + X_EachStuInformation[i - FirstStuInformation] + "”" + "行学生信息段后间距错误，应为0行");
                                if (!Util.correctfonts(list[i], doc, coverstyleStuInformation[1], "Times New Roman")) Util.printError("学生信息中" + "“" + X_EachStuInformation[i - FirstStuInformation] + "”" + "行学生信息字体错误，应为" + coverstyleStuInformation[1]);
                                if (!Util.correctsize(list[i], doc, coverstyleStuInformation[3])) Util.printError("学生信息中" + "“" + X_EachStuInformation[i - FirstStuInformation] + "”" + "行学生信息字号错误，应为" + coverstyleStuInformation[3]);
                            }
                        }
                        else NumLost++;
                    }
                ...
                if (YFlag == 0)
                {
                    if (!Util.correctJustification(list[i], doc, coverstyleSchoolNameCh[1])) Util.printError("封面中文学校名未居中");
                ...
                if (YFlag == 0)
                {
                    if (!Util.correctJustification(list[i], doc, coverstyleSchoolNameEn[1])) Util.printError("封面英文学校名未居中");
...
namespace PaperFormatDetection.Master
{
    class CoverStyle : Master.MasCoverStyle
    {
        ...
        {
            Init();
            MasterType = Util.masterType;

            List<Paragraph> list = covUtil.CovSectionLoction(doc, 1);
            detectCoverStyle(list, doc);
        }
        /// <summary>
        ...
        public void Init()
        {
            ...
            xmlDoc.Load(Util.environmentDir + "/Template/Master/Coverstyle.xml");
            int m = 0;
            //封面大标题
