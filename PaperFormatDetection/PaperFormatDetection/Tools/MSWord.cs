    ...
    class MSWord
    {
        ...
        public void getParagraphPage(/*string text*/)
        {
            ...
            foreach (Paragraph p in wordDoc.Paragraphs)
            {
               ...
               if (text.IndexOf('，')==-1 && text.Length>0  &&(Regex.IsMatch(text, @"^[表][ ]*?[0-9]") || chapterTitle.Contains(text) || Regex.IsMatch(text, @"^Fig")|| Regex.IsMatch(text, @"^Tab") || Regex.IsMatch(text, @"^[图][ ]*?[0-9]")))
                {
                    Title = text;
                    NumberOfPage = (int)p.Range.get_Information(WdInformation.wdActiveEndAdjustedPageNumber);
